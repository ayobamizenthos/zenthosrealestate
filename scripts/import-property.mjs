...
// The twelve areas the site actually lists in, island then mainland. Kept in
// step with PROPERTY_LOCATIONS in src/lib/constants.ts.
const LOCATIONS = [
  'Victoria Island',
  'Ikoyi',
  'Banana Island',
...
function parseFolderName(name) {
  ...
  if (!shape) {
    throw new Error(
      `Folder name must read "<n> Bedroom <Type> <Price>-<Area>, Lagos".
       Got: "${name}"`
    )
  }

  const [, bedsRaw, typeRaw, priceRaw, addressRaw] = shape
  ...
  return { bedrooms, propertyType, price, address, location }
}
...
const description = `${details.bedrooms} bedroom ${details.propertyType.toLowerCase()} at ${details.address}, Lagos. We inspected this home and verified its documents before listing it. Message us on WhatsApp to arrange a viewing.`
